...
interface Err<E extends Error> extends E.Left<E> {}
interface Ok<T> extends E.Right<T> {}

export type Result<E extends Error, T> = Err<E> | Ok<T>;

/** `Nothing : x -> Result x Never` */
export const Err = <E extends Error>(error: E): Result<E, never> => E.left(error);
...
/** `map4 :
       ...
       -> Result x value` */
export const map4 =
  <A, B = A, C = A, D = A, E = A>(fn: (a: A) => (b: B) => (c: C) => (d: D) => E) =>
  (m1: Result<Error, A>) =>
  (m2: Result<Error, B>) =>
  (m3: Result<Error, C>) =>
...
/** `map5 :
      ...
      -> Result x value` */
export const map5 =
  <A, B = A, C = A, D = A, E = A, F = A>(fn: (a: A) => (b: B) => (c: C) => (d: D) => (e: E) => F) =>
  (m1: Result<Error, A>) =>
  (m2: Result<Error, B>) =>
  (m3: Result<Error, C>) =>
  (m4: Result<Error, D>) => 
